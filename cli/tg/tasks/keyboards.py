...
async def all_tasks():
    ...
    for task in list_tasks:
        if task.is_completed is False:
            builder.add(InlineKeyboardButton(text=f"🟢{task.title}", callback_data=f"task_{task.id}"))
...
async def archive_tasks():
    ...
    builder.add(InlineKeyboardButton(text="На главную", callback_data="tasks_main"))
    builder.add(InlineKeyboardButton(text="Акутальные задачи", callback_data="listtasks"))
    return builder.adjust(1).as_markup()
...
async def info_task(title: str):
    builder = InlineKeyboardBuilder()
    task = await info_abt_task(title=title)
    if task is not None:
        if task.tag is not None:
            builder.add(InlineKeyboardButton(text=f"Тег: {task.tag}", callback_data="listtasks"))
        else:
            builder.add(InlineKeyboardButton(text="Тег отсутствует", callback_data="listtasks"))
        
        if task.is_completed is False:
            form = task.created_at.strftime("%d/%m/%Y")
            builder.add(InlineKeyboardButton(text=f"Дата создания: {form}", callback_data="listtasks"))
            builder.add(InlineKeyboardButton(text="Завершить задачу", callback_data=f"complete_{task.id}"))
            builder.add(InlineKeyboardButton(text="На главную", callback_data="tasks_main"))
            return builder.adjust(1,1,2,1).as_markup()
            
        else:
            form = task.expiring_date.strftime("%d/%m/%Y")
            builder.add(InlineKeyboardButton(text=f"Дата удаления: {form}", callback_data="listtasks"))
