...
async def tomain(callback: CallbackQuery):
    ...
    await callback.message.answer(text=f"Привет {callback.from_user.first_name}!", reply_markup=await src_kb.cards())
...
async def get_all(event: Message | CallbackQuery):
    ...
    if isinstance(event, Message):
        await event.answer(
            ...
            reply_markup=await kb.languages(tg=tg),
        )
...
async def start_learning(callback: CallbackQuery, state: FSMContext):
    ...
    words = await choosen_words(lang=lang, tg_id=tg)
    if not words:
...
async def translate_word(callback: CallbackQuery, state: FSMContext):
    ...
    tr_word = await choosen_words_tr(word=current_word, lang=current_language, tg_id=tg)
    await callback.message.edit_text(
...
async def continue_learning(callback: CallbackQuery, state: FSMContext):
    ...
    await change_card_status(word=current_word, num=int(callback.data.split("_")[-1]), tg_id=tg)
...
async def new_word(message: Message, state: FSMContext):
    ...
    await add_word(trword=trword, lang=lang, word=word, tg_id=tg)
    await status_msg.edit_text(
...
async def del_card_msg(event: Message | CallbackQuery, state: FSMContext):
    ...
    if isinstance(event, Message):
        ...
        text = []
        words = await cards_list(tg_id=tg)
        for word in words:
            text.append(f"ID: {word.id}, Слово: {word.word}")
        response_text = "\n".join(text)
        await event.answer(response_text)
        await event.answer(
            "Введите ID карточки или карточек через запятую которые вы хотите удалить\nПример №1: 43,45. Пример №2: 56"
        )
    elif isinstance(event, CallbackQuery):
        await event.answer()
        text = []
...
async def del_card(message: Message, state: FSMContext):
    ...
    await message.answer(f"Удалены карточки: {','.join(nums)} ", reply_markup=await kb.kb_show_start())
    await state.clear()
